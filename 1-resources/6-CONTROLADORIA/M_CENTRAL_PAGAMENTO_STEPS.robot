##################################################################################################################################
# Autor: Alan Patriota
# Decrição: Passos da tela de Central de Pagamento
##################################################################################################################################
*** Settings ***
#### Pages utilizadas na Suite de teste
Resource          ../../2-pages/6-CONTROLADORIA/M_CENTRAL_PAGAMENTO_PAGE.robot
Resource    M_FECH_CONT_STEPS.robot

*** Variable ***

*** Keywords ***

Escolher e Informar o Periodo |${DataIni}|
    Wait Until Element Is Visible      ${BotaoVencimento}        200
    Click no Item                      ${BotaoVencimento}
...
Selecionar Filtro e Pesquisar |${NrDoc}|
...
Informar Conta Corrente |${Conta}|
    Wait Until Element Is Visible    ${CampoConta}            200
    Preencher Campo                  ${CampoConta}            ${Conta}
    Send Keys    TAB
Validar Saldo |${Saldo}|
    Validar Elemento Pelo Titulo                 ${Saldo}
