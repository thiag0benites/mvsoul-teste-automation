...
Resource          ../../1-resources/0-JORNADA_PACIENTE/30-Realizar revisao de dados pelo aviso de cirurgia/RealizarRevisaoAvisoCirurgiaSteps.robot
Resource          ../../1-resources/0-JORNADA_PACIENTE/31-Transferencia Paciente para centro cirurgico/TransferenciaCentroCirurgicoSteps.robot
...
#SMF-9619:Realizar um pré-agendamento cirúrgico
   ${dados}   Seleciona massa de dados na suite "${suite}" do caso de teste "${filtro}"
    ...
    Clicar no botao [Reservar Sala]||
    Preencher os campos e reservar sala de cirurgia |${dados}[agCirDtHrPrevAlta]|
#SMF-9621:Consultar o cadastro do paciente para gerar uma internação
    ...
    Pesquisar paciente pelo prontuario |${dados}[cadPacOutputCodPaciente]|
...
#SMF-9644:Realizar Atendimento de Internação vinculando uma pré-internação por convênio particular
    ...
    Preencher campos da tela internacao do paciente |${dados}[preIntOrigem]|,|${dados}[preIntCodServico]|,|${dados}[preIntAcompanhante]|,|${dados}[intPacCodLeito]|,|${dados}[preAgDtHrPrevInter]|@nprint
...
#SCR1JSMK-002:Realizar revisão dos dados da cirurgia pelo aviso de cirurgia
    ${dados}    Seleciona massa de dados na suite "${suite}" do caso de teste "${filtro}"
    Acessar a tela "Atendimento>Centro Cirúrgico e Obstétrico>Centro Cirúrgico>Revisão Agendamento Cirurgia"@nprint @nlas
    Pesquisar paciente pelo aviso de cirurgia |${dados}[preAgCirAvisoCirurgia]|
    Clicar no botao [Aviso de Cirurgia]||
    Clicar no botao [Salvar]|${dados}[preIntMsgEsperada]|
    Clicar no botao [Sair]||
    Clicar no botao [Sair]||
#SMF-9622:Gerar um aviso de cirurgia
    ...
    # Clicar no botao [Salvar]|${dados}[preIntMsgEsperada]|
    # Captura do codigo do aviso de cirurgia|${suite}|${dados}[id]|
SMF-9624:Transferência de Paciente para Centro Cirúrgico e Rpa
    ${dados}    Seleciona massa de dados na suite "${suite}" do caso de teste "${filtro}"
...
#     Preencher os campos e confirmar |${dados}[cadPacOutputCodAtendimento]|,|${dados}[agCirPacMsgEsperada]|,|${dados}[agCirurEqpMedica]|,|${dados}[agCirurAnestesista]|,|${dados}[agCirurNumCid]|
...
#     Clicar no botao [Sair Tela]||
#     Clicar no botao [OK]|${dados}[preIntMsgEsperada]|,|${dados}[altaHospPacMgsEsperadaFinal]|
